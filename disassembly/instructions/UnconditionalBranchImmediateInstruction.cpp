...
UnconditionalBranchImmediateInstruction::UnconditionalBranchImmediateInstruction(uint32_t encoded) :
	is_with_link(encoded >> 31 & 1),
	immediate(UnconditionalBranchImmediateInstruction::decode_immediate(encoded)) {}

int32_t UnconditionalBranchImmediateInstruction::decode_immediate(uint32_t encoded) {

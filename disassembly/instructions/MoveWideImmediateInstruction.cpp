...
MoveWideImmediateInstructionOpType decode_op_type(uint32_t encoded) {
	const uint8_t opc_field = encoded >> 29 & 0b11;
	return static_cast<MoveWideImmediateInstructionOpType>(opc_field);
}
...
MoveWideImmediateInstruction::MoveWideImmediateInstruction(uint32_t encoded) :
